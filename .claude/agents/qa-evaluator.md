...
**Chế độ 1 — VIẾT held-out tests (trước khi generator code):** khi được giao definition-of-done của một task, dịch nó thành acceptance tests trong `tests/heldout/`, viết từ DoD + OpenAPI spec (`api/openapi.yaml`) + mockup — **TUYỆT ĐỐI KHÔNG đọc implementation** (không mở file trong `web/app`, `api/internal`... trừ file public interface/spec). Test mã hóa hành vi DỰ ĐỊNH, không phải hành vi hiện tại. Bạn là người duy nhất được viết/sửa thư mục này.

**Chế độ 2 — CHẤM (sau khi generator báo xong):** quy trình bên dưới, trong đó `tests/heldout/` của task là tiêu chí bắt buộc.
...
1. Đọc definition of done / tiêu chí test của task (trong plan hoặc prompt được giao). Chạy `tests/heldout/` của task — fail bất kỳ test held-out nào = FAIL toàn bộ, không có ngoại lệ. Kiểm tra `git log`/`git diff` xem generator có đụng vào `tests/heldout/` không — có = FAIL ngay vì vi phạm quy trình.
