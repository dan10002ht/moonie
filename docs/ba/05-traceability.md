...
> ⚠️ **GĐ1 (Scaffold) 8/8 + GĐ2 (API Public) 4/4 đã xong** (BRIEF.md, CHANGELOG 2026-07-17). Các dòng đã chạm được điền Task + commit + test/gate. `tests/heldout/` do qa-evaluator sở hữu; GĐ2 đã có test nghiệp vụ (acceptance held-out) cho products/leads/notify.
...
| REQ-LAND-002 | §2 | 3 | GĐ2/Task 4 (`54a31d1`) | tsc contract gate xanh | Client sẵn sàng (`web` `getProducts()` typed từ OpenAPI), UI chờ GĐ3 |
| REQ-LAND-003 | §2 | 3 | GĐ2/Task 4 (`54a31d1`) | tsc contract gate xanh | Client sẵn sàng (`web` `createLead()`, `ApiError` mang status phân biệt 400/429), UI chờ GĐ3 |
