...
- Lead mới **và đơn mới** (kể cả tạo trong admin): lưu DB + thông báo **Telegram Bot** tức thì (chốt 2026-07-17).
- Tồn kho = **trạng thái còn/hết hàng**, không đếm số lượng (bánh làm theo mẻ/đơn đặt; chốt 2026-07-17).
- Doanh thu tháng trên dashboard = **tổng đơn trạng thái `done`** trong tháng (tiền thực đã về); có thể hiển thị dòng phụ "đang xử lý" để tham khảo (chốt 2026-07-17).
- Convert lead → đơn: **KHÔNG tự tạo khách hàng** — bản ghi `customers` luôn tạo thủ công; đơn convert lấy tên/SĐT từ lead, gắn customer là bước thủ công tùy chọn → `orders.customer_id` nullable (chốt 2026-07-17).
...
**Admin (JWT httpOnly cookie, middleware auth):** mọi endpoint đều dưới tiền tố `/api/v1` — `POST /api/v1/auth/login`, CRUD `/api/v1/admin/products`, `/api/v1/admin/leads` (+ `POST .../leads/{id}/convert`), `/api/v1/admin/orders` (+ cập nhật trạng thái), `/api/v1/admin/customers`, `GET /api/v1/admin/dashboard` (đếm leads mới, đơn đang xử lý, doanh thu tháng). `api/openapi.yaml` là nơi chốt chính thức.
