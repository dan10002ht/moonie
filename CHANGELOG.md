...
- **Task 0** — Rate-limit theo IP client THẬT sau reverse proxy: `ClientIPResolver` (`api/internal/httpx/clientip.go`) — RemoteAddr không-trusted → bỏ qua X-Forwarded-For (chống spoof); trusted → lấy rightmost-non-trusted của XFF. Config `TRUSTED_PROXIES` (CSV IP/CIDR, fail-fast lúc boot). 2 rate limiter (leads 20/phút + login 10/phút) dùng chung resolver. Next forward XFF cho `/leads` + `/auth/login` (`web/lib/client-ip.ts`, `web/lib/api.ts`, `web/app/actions/{lead,admin}.ts`). Held-out 13/13 + go-reviewer PASS. ⚠️ prod BẮT BUỘC set TRUSTED_PROXIES. Commit `0c85951`.
- **Task 0b** — Security deploy-gate: seed từ chối default/weak password khi APP_ENV=production (`resolveSeedPassword`, ≥12 ký tự); `IsProduction()` case-fold (hết footgun `APP_ENV="Production"` tắt Secure cookie); header bảo mật toàn cục API (`securityHeaders`: nosniff + X-Frame-Options DENY + Referrer-Policy) + CSP/headers web (`next.config.ts`); CSRF Origin-check (`originCheck`) cho mutation admin + auth (Origin lạ → 403, fallback same-origin theo Host khi ALLOWED_ORIGIN rỗng, segment-aware prefix). Held-out 12/12 + go-reviewer PASS (3 minor đã sửa, gồm bug prefix `/adminx`).
- **Task 1** — Hạ tầng production + runbook: `docker-compose.prod.yml` (postgres nội bộ + migrate/seed one-shot + api + web + caddy; env fail-fast; TRUSTED_PROXIES subnet cố định; chỉ Caddy expose 80/443), `Caddyfile` (auto HTTPS + HSTS + same-origin proxy), `api/Dockerfile.migrate`, `scripts/backup.sh`+`restore.sh` (đã test round-trip thật), `docs/runbook.md` (deploy/rollback/restore/log/checklist go-live + 2 mốc security-review), `web/Dockerfile` (ARG NEXT_PUBLIC_API_BASE). Verify: compose config valid, backup/restore round-trip PASS. **Security-review milestone #2: 0 finding ≥8**. Còn lại (deploy VPS thật + TLS) cần tài nguyên chủ dự án.
